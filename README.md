...
Room *prices* are deliberately **not** part of that comparison, only the hotel names are. Real per-date pricing on this page drifts by small amounts even for what's otherwise the same package (a few hundred PKR here or there), so matching on exact prices caused near-identical dates to wrongly split into separate groups/messages instead of merging - matching on the hotel line-up instead fixes that, at the cost of the displayed price table being representative of the first date in the group rather than an exact per-date figure. A genuinely different airline, route, duration, or hotel selection still starts a new group, written as its own self-contained section (airline + duration, its date(s), baggage, hotel/price table) separated from the previous one by a blank line. The prefix/suffix settings above wrap the entire bulk message once, not each group individually.
