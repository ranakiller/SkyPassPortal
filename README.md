...
Each package card on `agents/book-umrah-packages-api` gets a "Copy" button next to its existing "PDF" link, producing a WhatsApp-ready message. The top of the message is deliberately minimal - airline name, the route (the page's own `data-sector` attribute, e.g. `` `LYP-DXB-JED-DXB-LYP` ``), and package duration all on one header line (`` `SV-Saudi Arabian Airlines LHE-JED-LHE *21 Days*` ``), then both flight legs (`` `FLIGHT DATE TIME` `` code blocks, same style used by every other copy button in this extension - airport codes are deliberately left off each leg line since the route's already on the header), then the baggage allowance line. No package price is shown up there. The card's available-seats count is appended as `` `💺 N` `` to just the first (departure) leg's line, since that's the one line per date that's actually specific to that date's inventory - in the bulk "Copy All" message (below), each merged date keeps its own seat count on its own first line even though the rest of that group's details are shared.
